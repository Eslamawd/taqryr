        ...
        <table class="container" width="600" cellpadding="0" cellspacing="0" bgcolor="#161b22" style="border-radius:16px; overflow:hidden;">
          ...
          <tr>
            <td class="padding-mobile" style="padding:30px; color:#e6edf3; line-height:1.8; font-size:15px;">
              ...
              <p style="margin:0 0 15px 0;">تم الاشتراك في الباقه بنجاح : {{ $sub->plan }}</p>
              <p style="margin:0 0 15px 0;">والباقه حاليا مفعله من تاريخ: 
              {{  optional($sub->start_date)->format('Y-m-d H:i') }}</p>
              <p style="margin:0 0 15px 0;">وسوف تننتهي في تاريخ: 
              {{ optional($sub->end_date)->format('Y-m-d H:i') }}</p>
              <p style="margin:0 0 15px 0;">وسعرها: {{ $sub->price }}</p>
